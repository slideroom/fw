...
var CloseStack = class {
  ...
  enroll(cb) {
    ...
    const close = (add2 = 0) => {
      ...
      if (idx >= 0) {
        for (let i = idx + add2; i < this.theCloseStack.length; i++) {
          this.theCloseStack[i]();
        }
        this.theCloseStack.splice(idx + add2);
      }
    };
    return {
      close: () => {
...
function initProvide(vm) {
  ...
  if (provideOption) {
    var provided2 = isFunction(provideOption) ? provideOption.call(vm) : provideOption;
    if (!isObject(provided2)) {
      return;
    }
    var source = resolveProvided(vm);
    var keys = hasSymbol ? Reflect.ownKeys(provided2) : Object.keys(provided2);
    for (var i = 0; i < keys.length; i++) {
      var key = keys[i];
      Object.defineProperty(source, key, Object.getOwnPropertyDescriptor(provided2, key));
    }
  }
}
...
function transformModel(options, data) {
  ...
  (data.attrs || (data.attrs = {}))[prop2] = data.model.value;
  var on = data.on || (data.on = {});
...
function validateProp(key, propOptions, propsData, vm) {
  ...
  if (value === void 0) {
    value = getPropDefaultValue(vm, prop2, key);
  ...
  if (true) {
    assertProp(prop2, key, value, vm, absent);
  }
  return value;
}
...
function assertProp(prop2, name, value, vm, absent) {
...
function prop(defaultValue = null) {
  return function(target2, key, descriptor) {
    const props2 = Reflect.getMetadata("view-engine:props", target2.constructor) || [];
    props2.push({ key, defaultValue });
    Reflect.defineMetadata("view-engine:props", props2, target2.constructor);
  };
}
function provided(defaultValue) {
  return function(target2, key, descriptor) {
    const props2 = Reflect.getMetadata("view-engine:provided", target2.constructor) || [];
    props2.push({ key, defaultValue });
    Reflect.defineMetadata("view-engine:provided", props2, target2.constructor);
  };
}
function getProps(cl) {
  var props2 = Reflect.getMetadata("view-engine:props", cl) || [];
  let propObject = {};
  props2.forEach((p) => {
    propObject[p.key] = {
...
var makeComputedObject = (instance) => {
  const obj = {};
...
var getTemplateFor = (viewModel) => {
  ...
  for (let key in __webpack_require__.c) {
    ...
    for (let exportKey in exports) {
      ...
      const template2 = exports.__html;
      return template2;
    }
  }
  throw new Error(`Can't find template for: ${viewModel.name}`);
};
var makeVueComponent = (viewModel, onInstanceCreated = null, overrider = null) => {
  const props2 = getProps(viewModel);
  const provided2 = getProvided(viewModel);
  return Vue.extend({
    ...
    data: function() {
      ...
      for (let m of Object.getOwnPropertyNames(instance.constructor.prototype)) {
        if (this.$options.computed[m] === void 0 && typeof instance[m] == "function" && m != "constructor" && !specialMethods[m]) {
          const boundFn = instance[m].bind(this);
          this.$options.methods[m] = boundFn;
          this[m] = boundFn;
        }
      }
      const needs2 = Reflect.getMetadata("components", viewModel);
      if (needs2) {
        const components = {};
        needs2.forEach((need) => {
    ...
    created: function() {
      ...
      for (let propName of Object.getOwnPropertyNames(props2)) {
...
var makeAndActivate = async (viewModel, where, activateData = null, overrider = null) => {
...
var Bus = class {
  ...
  subscribe(type, cb) {
    ...
    return {
      dispose: () => {
        let listeners2 = this.listeners.get(type);
        if (listeners2 == null) return;
        const idx = listeners2.indexOf(cb);
        if (idx > -1) {
...
var bus = ContainerInstance.get(Bus);
var ViewRouterLocationChanged = class {
...
var ViewRouter = class {
  constructor(viewEngine2, starter) {
  ...
  async runMatching(location, fullLocation, queryParams, loadedView, viewStackIndex) {
    ...
    if (hasMoreInStack) {
      ...
      if (arrayEqual(nextLView.matchedOn, match.matchedOn) && nextLView.queryParams == matchQueryParams) {
        const idx2 = viewStackIndex + 1;
        await this.runMatching(
          match.remaining,
          fullLocation,
          queryParams,
          this.loadedViewsStack[idx2],
          idx2
        );
        return;
      }
...
var AResponseContext = class {
  ...
  parseHeaders() {
    ...
    this.request.getAllResponseHeaders().trim().split("\n").forEach((line) => {
      ...
      const key = line.slice(0, index2).trim().toLowerCase();
  ...
  doRequest(method, url, params, content) {
    return new Promise((res, rej) => {
      ...
      p.addEventListener("loadend", (e) => {
        ...
        const responseContext = new AResponseContext(
          p,
          parsedRes
        );
        if (this.middleware.length > 0) {
...
// src/fw.ts
function inject(target2) {
  return;
}
function needs(...things) {
  return function(target2) {
    Reflect.defineMetadata("components", things, target2);
  };
}
var viewEngine = new ViewEngine(ContainerInstance);
var FrameworkConfig = class {
  starter = null;
  startWith(view) {
    this.starter = view;
  }
...
async function bootstrap(cb) {
  ...
  const bus2 = ContainerInstance.get(Bus);
  await cb(fwConfig);
